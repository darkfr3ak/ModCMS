...
class Base {
    ...
    public function getAllApps() {
        $sql = "SELECT * FROM mycms_core_apps";
	$db = $this->getDbo();				
    ...
    public function getSiteSettings() {
        $db = $this->getDbo();
        $sql = "SELECT * FROM mycms_core_settings";
        $rows = $db->loadResult($sql);
    ...
    public function installApp($app_data = array()) {
        $sql = "INSERT INTO mycms_core_apps VALUES
                    (NULL,
    ...
    public function removeApp($app_data = array()) {
        $sql = "DELETE FROM mycms_core_apps WHERE app_name = '".$app_data->app->app_data->app_name."'";
        $db = $this->getDbo();				
        if($db->query($sql)){
    ...
    public function getLocalApps($directory = "apps") {
    ...
    */
    ...
    public function getActiveWidgets() {
        $db = $this->getDbo();
        $sql = "SELECT mycms_widgets_active.active_position, mycms_widgets_all.widget_name FROM mycms_widgets_active, mycms_widgets_all WHERE mycms_widgets_active.active_widget = mycms_widgets_all.widget_id;";
        $rows = $db->loadResult($sql); 
        return $rows;
    }
    
    public function setActiveWidget($param) {
        $sql = "INSERT INTO mycms_widgets_active VALUES('$title','$desc')";
	$db = $this->getDbo();				
	if($db->query($sql)){
            return true;
        }
    ...
    public function getWigdets() {
        $db = $this->getDbo();
        $sql = "SELECT * FROM mycms_widgets_all";
        $rows = $db->loadResult($sql);
        return $rows; 
    }
    ...
    public function getWigdetPositions() {
        $db = $this->getDbo();
        $sql = "SELECT * FROM mycms_widgets_positions";
        $rows = $db->loadResult($sql);
        return $rows; 
    }
    ...
    public function widgetOutput($position='default'){
        if(!empty($this->widgetPositions[$position])){
            ...
            foreach($widgets as $widgetObject){
                ...
                $file = 'widgets/'.$widgetName.'/'.$widgetName.'.widget.php';
                if(file_exists($file)){
                    require_once($file);
                    ...
                    trigger_error("Widget not found: ".ucfirst($appname), E_USER_ERROR);
                }
            }
        }
    }
