...
if (realpath(__FILE__) == realpath($_SERVER['DOCUMENT_ROOT'].$_SERVER['SCRIPT_NAME'])){
    ...
    if($_FILES["zip_file"]["name"]) {
        ...
        $target_path = SITE_ROOT."upload/".$filename;  // change this to the correct site path
        if(move_uploaded_file($source, $target_path)) {
            ...
            if ($x === true) {
                switch ($name[1]) {
                    case "app":
                        $zip->extractTo(SITE_ROOT."apps/"); // change this to the correct site path
                        $success = true;
                        break;
                    case "widget":
                        $zip->extractTo(SITE_ROOT."widgets/"); // change this to the correct site path
                        $success = true;
                        break;
                    case "theme":
            ...
            <div class="panel-heading">
                ...
                <span class="pull-right">
                    ...
                    <ul class="nav panel-tabs">
                        <li class="active"><a href="#tab1" data-toggle="tab">App hochladen</a></li>
                        <li><a href="#tab2" data-toggle="tab">Apps installieren</a></li>
                        <li><a href="#tab3" data-toggle="tab">Installierte Apps</a></li>
                    </ul>
                </span>
            </div>
                                ...
                                <table class="table table-striped">
                                    ...
                                    foreach ($notInstalled as $key => $value) {
                                        if($value["installed"] == "false"){
                                            $str="apps/".$value["dir"]."/".$value["dir"].".app.xml";
                                            $sxml = simplexml_load_file($str);
                                            
                                            echo "<tr>";
                                            echo "<td><i class='fa fa-".$sxml->app->app_data->app_icon."'></i></td>";
                    ...
                    <div class="tab-pane" id="tab3">
                        <div class="table-responsive">
                                <table class="table table-striped">
                                    ...
                                    for ($index = 0; $index < count($allApps); $index++) {  
                                        $str="apps/".$allApps[$index]->app_name."/".$allApps[$index]->app_name.".app.xml";
                                        $sxml = simplexml_load_file($str);                                    
                                        echo "<tr>";
