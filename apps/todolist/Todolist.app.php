...
class TodolistApp extends ApplicationBase {
	...
	$desc = $_REQUEST['desc'];
	$sql = "INSERT INTO mycms_todolist_tasks VALUES(NULL,'$title','$desc')";
	$db = $this->getDbo();				
	if($db->query($sql)){				
    ...
    public function viewtodolist(){
        $db = $this->getDbo();
        $sql = "SELECT * FROM mycms_todolist_tasks";
	...
	$id = $_REQUEST['taskid'];
	$db = $this->getDbo();
	$sql = "DELETE FROM mycms_todolist_tasks WHERE id=$id";
	$db->query($sql);
	...
	$id = $_REQUEST['taskid'];
	$sql = "SELECT * FROM mycms_todolist_tasks WHERE id=$id";
	$db = $this->getDbo();
	...
	$title = $_REQUEST['title'];
	$desc = $_REQUEST['desc'];
	$sql = "UPDATE mycms_todolist_tasks as tasks SET tasks.title='$title',tasks.desc='$desc' WHERE id=$id";
	$db = $this->getDbo();				
	if($db->query($sql)){					
            $this->redirect('index.php?app=todolist&task=viewtodolist');				
	}else{				
